...
- **`model.to(torch.bfloat16)`（BF16）**：将模型权重转换为 bfloat16，显著降低显存占用并提升吞吐。与 FP16 做比较，BF16 具备更大指数范围，能够更好地处理由梯度消失或爆炸引发的数值上溢（Overflow）和下溢（Underflow）问题，从而具备更好的数值稳定性。在“不出错”比“算得精”更重要的训练场景下，BF16 更好。
