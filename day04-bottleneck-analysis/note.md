...
| **模型配置** | **Qwen-7B** (4-bit 量化后约 4GB) | **Qwen-72B** (FP16 约 140GB, 需 2x H800) |
...
- **`model.half()`（FP16）**：将整个模型参数统一转换为半精度（float16），降低显存占用并提升计算速度，但可能带来数值稳定性问题。

- **`torch.amp.autocast()`（AMP）**：在运行时按算子自动选择 FP16 或 FP32，实现性能与数值稳定性的平衡，是更推荐的混合精度方案。

- **注**：AMP 只优化计算精度而不降低模型权重显存占用，因此大模型推理必须先用 FP16/BF16 加载权重，再配合 AMP 才能同时避免 OOM 并获得性能与稳定性的平衡。
