...
## 3. 核心映射原理（线性量化）
### 1. 核心逻辑
- 通过 scale（缩放因子）+ zero_point（零点），完成浮点→整数的双向映射（量化+反量化），支持直接低精度计算。
- 利用 “线性映射可交换” 的数学特性，直接用低精度（INT8/INT4）完成计算，仅在进入精度敏感环节（如 QK 相似度、softmax、alpha*V、LN）前，反量化回 FP16，兼顾效率与精度。
### 2. 两种量化方式
